 ...
 * Riceve un oggetto config (schema in docs/tool-config-schema.md) e un
 * registry di funzioni di calcolo, e monta l'intera pagina: hero+tool,
 * spiegazione, esempi, FAQ, related tools, JSON-LD.
 *
 * In produzione (Next.js) questa logica si divide in Server Components
 * (SEO, spiegazione, esempi, FAQ, related — statici, ottimi per SEO) e
 * un piccolo Client Component solo per il tool interattivo.
 */

const CALCULATORS = {}; // registry: logicId -> funzione pura (inputs) => outputs
